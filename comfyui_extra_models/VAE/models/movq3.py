...
class SpatialNorm(nn.Module):
    def __init__(
            self, f_channels, zq_channels=None, norm_layer=nn.GroupNorm, freeze_norm_layer=False, add_conv=False,
            **norm_layer_params
    ):
        super().__init__()
    ...
    def forward(self, f, zq=None):
        ...
        if zq is not None:
            f_size = f.shape[-2:]
            zq = torch.nn.functional.interpolate(zq.float(), size=f_size, mode="nearest").to(zq.dtype)
            if self.add_conv:
                zq = self.conv(zq)
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.interpolate(x.float(), scale_factor=2.0, mode="nearest").to(x.dtype)
        if self.with_conv:
            x = self.conv(x)
        return x
