...
class EXVAE(comfy.sd.VAE):
    ...
    def decode_tiled_(self, samples, tile_x=64, tile_y=64, overlap=16):
        ...
        output = torch.clamp((
                                     (comfy.utils.tiled_scale(samples, decode_fn, tile_x // 2, tile_y * 2, overlap,
                                                              upscale_amount=self.latent_scale, pbar=pbar) +
                                      comfy.utils.tiled_scale(samples, decode_fn, tile_x * 2, tile_y // 2, overlap,
                                                              upscale_amount=self.latent_scale, pbar=pbar) +
                                      comfy.utils.tiled_scale(samples, decode_fn, tile_x, tile_y, overlap,
                                                              upscale_amount=self.latent_scale, pbar=pbar))
    ...
    def encode_tiled_(self, pixel_samples, tile_x=512, tile_y=512, overlap=64):
        ...
        samples = comfy.utils.tiled_scale(pixel_samples, encode_fn, tile_x, tile_y, overlap,
                                          upscale_amount=(1 / self.latent_scale), out_channels=self.latent_dim,
                                          pbar=pbar)
    ...
    def decode(self, samples_in):
        ...
        try:
            ...
            pixel_samples = torch.empty((samples_in.shape[0], 3, round(samples_in.shape[2] * self.latent_scale),
                                         round(samples_in.shape[3] * self.latent_scale)), device="cpu")
    ...
    def encode(self, pixel_samples):
        ...
        try:
            ...
            batch_number = int(free_memory / memory_used)
            batch_number = max(1, batch_number)
            samples = torch.empty((pixel_samples.shape[0], self.latent_dim,
                                   round(pixel_samples.shape[2] // self.latent_scale),
                                   round(pixel_samples.shape[3] // self.latent_scale)), device="cpu")
